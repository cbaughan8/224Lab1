...
void printDataAsHex(unsigned char *data, size_t size) {
   ...
   for (unsigned i = 0; i < 16; ++i){
	if (size > i) printf("%02x", data[i]);
	else if (size <= i) printf("  ");
	if(((i + 1) % 2 == 0) && !((i + 1) % 16 == 0)) printf(" ");
   }
}
...
void printDataAsChars(unsigned char *data, size_t size) {
   for (unsigned i = 0; i < size; ++i){
        if(data[i] < 32 || data[i] > 126) printf(".");
...
void printDataAsCharsForBinary(unsigned char *data, size_t size) {
   for (unsigned i = 0; i < size; ++i){
        if(data[i] < 32 || data[i] > 126) printf(".");
        else printf("%c", data[i]);
   }
...
void printDataAsBits(unsigned char *data, size_t size){
   ...
   for (unsigned i = 0; i < 6; ++i){
	...
	if(!((i + 1) % 6 == 0)) printf(" ");
        if (size <= i) printf("        "); //8 spaces for 8 bits
   }   
...
void readAndPrintInputAsBits(FILE *input) {
   ...
   while (numBytesRead != 0) {
      ...
      printf("  ");
      printDataAsCharsForBinary(data, numBytesRead);
      printf("\n");
